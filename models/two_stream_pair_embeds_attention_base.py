...
def model(
    ...
    # Setup for DAGE loss
    model_att   = model_attention(input_shape=get_output_shape(model_base), embed_size=attention_embed_size)
    model_att_p = model_attention(input_shape=get_output_shape(model_base), embed_size=attention_embed_size)
    concat_out = keras.layers.Concatenate(axis=0)([base_out_src, base_out_tgt])
