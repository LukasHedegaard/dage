...
class AngularLinear(Layer):
    ...
    def build(self, input_shape):
        ...
        if not (dtype.is_floating or dtype.is_complex):
            raise TypeError('Unable to build `AngularLinear` layer with non-floating point dtype %s' % (dtype,))
        input_shape = tensor_shape.TensorShape(input_shape) # N x D
        self.dims = tensor_shape.dimension_value(input_shape[-1])
        if self.dims is None:
            raise ValueError('The last dimension of the inputs to `AngularLinear` should be defined. Found `None`.')
    ...
    def call(self, inputs):
        ...
        if rank != 2:
            raise ValueError('The input to `AngularLinear` should be of rank 2, but got input of shape %s'% inputs.shape)
