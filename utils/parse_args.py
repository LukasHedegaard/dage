...
def parse_args(raw_args=None):
    ...
    info_parser.add_argument(
        "--experiment_id", type=str, default="", help="Experiment ID"
    )
    ...
    os_parser.add_argument(
        ...
        type=int,
        default=0,
        help="Delete checkpoint after model training. Default: 0",
    )
    ...
    model_parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of unfrozen layers in base network. Default: -1 (all)",
    )
    model_parser.add_argument(
        "--embed_size",
        type=int,
        default=128,
        help="Size of embedding layer. Default: 128",
    )
    model_parser.add_argument(
        "--dense_size",
        type=int,
        default=1024,
        help="Size of first dense layer. Default: 1024",
    )
    model_parser.add_argument(
        "--l2",
        type=float,
        default=1e-4,
        help="L2 normalisation parameter. Default: 1e-4",
    )
    model_parser.add_argument(
        "--batch_norm",
        type=int,
        default=1,
        help="Use batch normalisation layers. Default: 1",
    )
    model_parser.add_argument(
        "--dropout", type=float, default=0.25, help="Dropout. Default: 0.25"
    )
    model_parser.add_argument(
        "--aux_dense_size",
        type=int,
        default=31,
        help="Size of aux dense layer. Default: 31",
    )
    model_parser.add_argument(
        "--architecture",
        type=str,
    ...
    loss_parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="Weighting for distance-based domain adaption losses. Default: 0.25",
    )
    loss_parser.add_argument(
        "--loss_weights_even",
        type=float,
        default=0.5,
        help="ratio of source to target loss weighting. For a value of 0, only the source is weighted.",
    )
    loss_parser.add_argument(
        "--connection_type",
        type=str,
    ...
    loss_parser.add_argument(
        ...
        "KSD: Edges for the k shortest distances are used;"
    ...
    loss_parser.add_argument(
        ...
        type=float,
        default=1,
        help="Parameter for the connection_filter_type. If loss only takes a single parameter (margin for CCSA and d-SNE), it is specified here. Default: 1",
    )
    loss_parser.add_argument(
        "--penalty_connection_filter_param",
        type=float,
        default=1,
        help="Parameter for the penalty_connection_filter_type. Default: 1",
    )
    loss_parser.add_argument(
        "--attention_activation",
        type=str,
        default="softmax",
        help="Activation function for attention. Default: softmax",
    )
    ...
    train_parser.add_argument(
        "--training_regimen",
        type=str,
        default="batch_repeat",
        help="How train the mode:"
        "regular: Train model using a regular model.fit;"
        "batch_repeat: Train model using a regular model.fit but repeating each batch;"
    ...
    train_parser.add_argument(
        ...
        type=int,
        default=8,
        help="Parameter only used for digits data.",
    )
    train_parser.add_argument(
    ...
    train_parser.add_argument(
        "--batch_size", "-b", type=int, default=16, help="Batch size. Default: 16"
    )
    train_parser.add_argument(
        "--epochs", "-e", type=int, default=10, help="Number of epochs. Default: 10"
    )
    train_parser.add_argument(
        "--seed",
        type=int,
        default=None,
        help="Random seed. If no argument is given, a random seed is used.",
    )
    train_parser.add_argument(
        "--augment",
        type=int,
        default=0,
        help='Activate augmentation. Only works with features="images". Default: 0',
    )
    train_parser.add_argument(
        "--standardize_input",
        type=int,
        default=0,
        help="Standardize input features. Default: 0",
    )
    train_parser.add_argument(
        "--resize_mode",
        type=int,
    ...
    train_parser.add_argument(
        ...
        type=float,
        default=1,
        help="Ratio of negative to positive pairs for domain adaption. Default: 1",
    )
    train_parser.add_argument(
        "--shuffle_buffer_size",
        type=int,
        default=5000,
        help="Size of buffer used for shuffling",
    )
    train_parser.add_argument(
        "--verbose", type=int, default=1, help="Verbosity of training and evaluation"
    )
    train_parser.add_argument(
        "--batch_repeats",
        type=int,
        default=1,
        help="Number of times to repeat the same batch during training. Default: 1",
    )
    train_parser.add_argument(
        "--test_as_val",
        type=int,
        default=0,
        help="Use test data for validation. Default: 0",
    )
    train_parser.add_argument(
        "--val_as_test", type=int, default=0, help="Use val data for test. Default: 0"
    )
    ...
    optim_parser.add_argument(
        "--learning_rate_decay", type=float, default=0.0, help="Learning rate decay"
    )
