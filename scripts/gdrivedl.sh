#!/bin/bash
# Download large files from Google Drive without having to manually confirm
# Author: https://www.matthuisman.nz/2019/01/download-google-drive-files-wget-curl.html
...
if [[ ! $(find "$tmp_file" -type f -size +10000c 2>/dev/null) ]]; then
   confirm=$(cat "$tmp_file" | sed -En 's/.*confirm=([0-9A-Za-z_]+).*/\1/p')
fi
...
[ -z "$filename" ] && filename=$(cat "$tmp_headers" | sed -En 's/.*filename=\"(.*)\".*/\1/p')
